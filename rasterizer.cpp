...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    int xlow = std::max(0, int(xmin)),
        ylow = std::max(0, int(ymin)),
        xupp = std::min(width, int(xmax) + 1),
        yupp = std::min(height, int(ymax) + 1); // round up would bring error, e.g, 4.4 -> 4 causes ignoring the domain (4, 4.4) 
    float x, y;
